...
execute if block ~ ~-1 ~ air if block ~ ~-2 ~ air if block ~ ~-3 ~ air if score @s vel_y matches 120.. run scoreboard players remove @s vel_y 50
execute if block ~ ~-1 ~ air if block ~ ~-2 ~ air if block ~ ~-3 ~ air run scoreboard players remove @s vel_y 30
execute if block ~ ~-1 ~ air if block ~ ~-2 ~ air run scoreboard players remove @s vel_y 20
...
#execute positioned ~-1 ~-.01 ~-1 if entity @e[type=block_display,tag=block,tag=stationary,dx=1,dy=0.01,dz=1] run scoreboard players set @s vel_y 0
#execute positioned ~-1 ~-.01 ~-1 if entity @e[type=block_display,tag=block,tag=stationary,dx=1,dy=0.01,dz=1] run tp @s ~1 ~0.03 ~1
execute positioned ~-1 ~-1.21 ~-1 if entity @e[type=block_display,tag=block,tag=stationary,dx=1,dy=1.21,dz=1] run function scale:block/push_out

execute if score @s vel_x matches 0 if score @s vel_z matches 0 if score @s vel_y matches -1..1 unless block ~ ~-.001 ~ air if block ~ ~ ~ air run tag @s add stationary
execute if score @s vel_x matches 0 if score @s vel_z matches 0 if block ~ ~ ~ air positioned ~-1 ~-1.01 ~-1 if entity @e[type=block_display,tag=block,tag=stationary,dx=1,dy=1.01,dz=1] run tag @s add stationary
...
execute as @s[tag=stationary] as @e[type=shulker,tag=block] if score @s id = $check id run tag @s add force_ride 
execute as @s[tag=stationary] unless entity @e[type=shulker,tag=block,tag=!force_ride,distance=..0.1] run ride @e[type=shulker,tag=block,tag=force_ride,limit=1,sort=nearest] mount @s 
tag @e[type=shulker,tag=block] remove force_ride