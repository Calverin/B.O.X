tag @s add temp
execute if block ~ ~-0.1 ~ air positioned ~-1 ~-1.01 ~-1 unless entity @e[type=block_display,tag=block,tag=!shulker,tag=!temp,tag=stationary,dx=1,dy=1.01,dz=1] run tag @s remove stationary
