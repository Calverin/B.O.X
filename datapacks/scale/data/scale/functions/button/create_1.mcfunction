...
summon block_display ~ ~-0.8 ~ {Tags:["scale","button","new_button","no_connection"],block_state:{Name:"minecraft:red_concrete"},brightness:{sky:15,block:15},Glowing:1b}
execute as @e[type=block_display,tag=new_button] at @s run data merge entity @s {width:1,height:1,transformation:{scale:[1.25f, 0.1f, 1.25f],translation:[-0.75f,0.8f,-0.75f]},interpolation_start:-2}
